...
class KobisDBLoader:
    ...
    def step1_load_raw_data(self, df):
        ...
        for i in range(0, total_rows, batch_size):
            ...
            for _, row in df.iloc[i:i+batch_size].iterrows():
                batch_data.append((
                    str(row['영화명']), str(row['영화명(영문)']), str(row['제작연도']),
                    str(row['제작국가']), str(row['유형']), str(row['장르']),
                    str(row['제작상태']), str(row['감독']), str(row['제작사']),
    ...
    def step2_normalize_data(self):
        ...
        SELECT DISTINCT TRIM(director) FROM movie_info WHERE director IS NOT NULL AND director != ''
        """)
        self.connection.commit()
        ...
        SELECT DISTINCT TRIM(production_company) FROM movie_info WHERE production_company IS NOT NULL AND production_company != ''
        """)
        self.connection.commit()
        ...
        for genre in all_genres:
            if genre: self.cursor.execute("INSERT IGNORE INTO genres (name) VALUES (%s)", (genre,))
        self.connection.commit()
        ...
        for (countries,) in self.cursor.fetchall():
            all_countries.update(c.strip() for c in countries.split(','))
        for country in all_countries:
            if country: self.cursor.execute("INSERT IGNORE INTO countries (name) VALUES (%s)", (country,))
        self.connection.commit()
        ...
        SELECT
            ...
            CASE
                ...
                ELSE NULL
            END,
            mi.type, mi.production_status, d.director_id, pc.company_id
        FROM movie_info mi
        ...
        print("  - 영화-장르 관계 설정 중...")
        self.cursor.execute("""
        SELECT m.movie_id, mi.genres FROM movies m
        JOIN movie_info mi ON m.title_ko = mi.title_ko AND m.title_en = mi.title_en
        WHERE mi.genres != ''
        """)
        movie_genres_data = []
        for movie_id, genres in self.cursor.fetchall():
            for genre in [g.strip() for g in genres.split(',') if g.strip()]:
                ...
                if result: movie_genres_data.append((movie_id, result[0]))
        if movie_genres_data:
            batch_size = 5000
            for i in range(0, len(movie_genres_data), batch_size):
                self.cursor.executemany("INSERT IGNORE INTO movie_genres (movie_id, genre_id) VALUES (%s, %s)", movie_genres_data[i:i+batch_size])
                self.connection.commit()

        print("  - 영화-국가 관계 설정 중...")
        self.cursor.execute("""
        SELECT m.movie_id, mi.production_countries FROM movies m
        JOIN movie_info mi ON m.title_ko = mi.title_ko AND m.title_en = mi.title_en
        ...
        for movie_id, countries in self.cursor.fetchall():
            for country in [c.strip() for c in countries.split(',') if c.strip()]:
                ...
                result = self.cursor.fetchone()
                if result: movie_countries_data.append((movie_id, result[0]))
        if movie_countries_data:
            batch_size = 5000
            for i in range(0, len(movie_countries_data), batch_size):
                self.cursor.executemany("INSERT IGNORE INTO movie_countries (movie_id, country_id) VALUES (%s, %s)", movie_countries_data[i:i+batch_size])
                self.connection.commit()
    ...
    def get_statistics(self):
        ...
        for label, query in stats_queries:
            ...
            result = self.cursor.fetchone()
            if result: print(f"  - {label}: {result[0]:,}개")
